...
export default async function PrivacyPage({ params }) {
  ...
  const privacy = await getTranslations(locale, 'privacy')
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1">
        <div className="max-w-4xl mx-auto px-4 py-16">
          {/* 标题 */}
          <div className="mb-12">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">{privacy.title}</h1>
            <p className="text-gray-600">{privacy.lastUpdated}</p>
          </div>
          ...
          <div className="space-y-8">
            {Object.keys(privacy.sections).map((key) => (
              <div key={key} className="bg-white rounded-xl p-8 shadow-sm">
                <h2 className="text-2xl font-semibold text-gray-900 mb-4">
                  {privacy.sections[key].title}
                </h2>
                <div className="text-gray-600 leading-relaxed whitespace-pre-line">
                  {privacy.sections[key].content}
                </div>
              </div>
            ))}
          </div>
